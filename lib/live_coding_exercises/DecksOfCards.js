...
function solution(A) {
    ...
    let decks = Object.keys(map).map(card => { return map[card];});
    return Object.keys(map).length < 52 ? 0: Math.min.apply(this, decks);
}

function solutionQuestionable(A) {
    var map = {};
    ...
    return decks.length < 52 ? 0 : Math.min.apply(0, decks);
}
